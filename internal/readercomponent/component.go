...
func Compile(holder reflect.Type, packageName string, inputType, outputType reflect.Type, resources *resource.Store, connector *dsql.SQLComponent) (*registry.RegisteredComponent, dexec.ComponentTarget, error) {
	...
	artifact, err := bootstrap.BuildArtifact(bootstrap.ArtifactInput{Component: component,
		InputType: inputType, OutputType: outputType, Resources: resources})
	if err != nil {
		return nil, dexec.ComponentTarget{}, err
	}
