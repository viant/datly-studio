package store_head_test

//go:generate env DATLY_GENERATE_GENERATION_HEAD=1 go test -run TestGenerateGenerationHead -count=1 .
...
func TestGenerateGenerationHead(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/runtime_generations/store_head")
	payload, err := os.ReadFile(filepath.Join(directory, "head.dql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/runtime_generations/store_head",
		Name:  "head", Path: "head.dql", Text: string(payload), Connector: "studio",
	...
	if os.Getenv("DATLY_GENERATE_GENERATION_HEAD") == "1" {
		destination = root
