package store_readers_test

//go:generate env DATLY_GENERATE_RUNTIME_READERS=1 go test -run TestGenerateRuntimeReaders -count=1 .
...
func TestGenerateRuntimeReaders(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/runtime_generations/store_readers")
	payload, err := os.ReadFile(filepath.Join(directory, "reader.dql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/runtime_generations/store_readers",
		Name:  "reader", Path: "reader.dql", Text: string(payload), Connector: "studio",
	...
	if os.Getenv("DATLY_GENERATE_RUNTIME_READERS") == "1" {
		destination = root
