package store_catalog_test

//go:generate env DATLY_GENERATE_GENERATION_CATALOG=1 go test -run TestGenerateGenerationCatalog -count=1 .
...
func TestGenerateGenerationCatalog(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/runtime_generations/store_catalog")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/runtime_generations/store_catalog",
	...
	if os.Getenv("DATLY_GENERATE_GENERATION_CATALOG") == "1" {
		destination = root
