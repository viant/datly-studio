package store_state_test

//go:generate env DATLY_GENERATE_GENERATION_STATE=1 go test -run TestGenerateGenerationState -count=1 .
...
func TestGenerateGenerationState(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/runtime_generations/store_state")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/runtime_generations/store_state",
	...
	if os.Getenv("DATLY_GENERATE_GENERATION_STATE") == "1" {
		destination = root
