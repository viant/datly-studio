...
//go:generate env DATLY_GENERATE_RUNTIME_STATUS=1 go test -run TestGenerateRuntimeStatus -count=1 .
...
func TestGenerateRuntimeStatus(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/runtime_generations/store_status")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/runtime_generations/store_status",
	...
	if os.Getenv("DATLY_GENERATE_RUNTIME_STATUS") == "1" {
		destination = root
