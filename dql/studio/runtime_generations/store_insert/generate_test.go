...
//go:generate env DATLY_GENERATE_GENERATION_INSERT=1 go test -run TestGenerateGenerationInsert -count=1 .
...
func TestGenerateGenerationInsert(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/runtime_generations/store_insert")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/runtime_generations/store_insert",
	...
	if os.Getenv("DATLY_GENERATE_GENERATION_INSERT") == "1" {
		destination = root
