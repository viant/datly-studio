package store_config_test

//go:generate env DATLY_GENERATE_REPORT_CONFIG=1 go test -run TestGenerateReportConfig -count=1 .
...
func TestGenerateReportConfig(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/reports/store_config")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/reports/store_config",
	...
	if os.Getenv("DATLY_GENERATE_REPORT_CONFIG") == "1" {
		destination = root
