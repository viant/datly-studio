package store_global_access_test

//go:generate env DATLY_GENERATE_GLOBAL_ACCESS=1 go test -run TestGenerateGlobalAccess -count=1 .
...
func TestGenerateGlobalAccess(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/reports/store_global_access")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/reports/store_global_access",
	...
	if os.Getenv("DATLY_GENERATE_GLOBAL_ACCESS") == "1" {
		destination = root
