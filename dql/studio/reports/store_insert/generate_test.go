...
//go:generate env DATLY_GENERATE_REPORT_INSERT=1 go test -run TestGenerateReportInsert -count=1 .
...
func TestGenerateReportInsert(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/reports/store_insert")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/reports/store_insert",
	...
	if os.Getenv("DATLY_GENERATE_REPORT_INSERT") == "1" {
		destination = root
