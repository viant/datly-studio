package store_draft_pointer_test

//go:generate env DATLY_GENERATE_DRAFT_POINTER=1 go test -run TestGenerateDraftPointerWriter -count=1 .
...
func TestGenerateDraftPointerWriter(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/reports/store_draft_pointer")
	payload, err := os.ReadFile(filepath.Join(directory, "report.dql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/reports/store_draft_pointer",
		Name:  "report", Path: "report.dql", Text: string(payload), Connector: "studio",
	...
	if os.Getenv("DATLY_GENERATE_DRAFT_POINTER") == "1" {
		destination = root
