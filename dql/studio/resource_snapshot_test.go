...
//go:generate env DATLY_GENERATE_RESOURCE_SNAPSHOT=1 go test -run TestGenerateResourceSnapshotReaders -count=1 .
...
func TestGenerateResourceSnapshotReaders(t *testing.T) {
	...
	for _, item := range []struct{ table, name string }{
		{"report_resource_files", "file"}, {"report_resource_folders", "folder"}, {"report_skill_roots", "skill"},
	} {
		directory := filepath.Join(root, "dql/studio", item.table, "store_snapshot")
		...
		compiled, compileErr := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
			Scope: "github.com/viant/datly-studio/dql/studio/" + item.table + "/store_snapshot",
		...
		if os.Getenv("DATLY_GENERATE_RESOURCE_SNAPSHOT") == "1" {
			destination = root
