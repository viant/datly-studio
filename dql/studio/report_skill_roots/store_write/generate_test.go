...
//go:generate env DATLY_GENERATE_SKILL_ROOT_WRITE=1 go test -run TestGenerateSkillRootWrite -count=1 .
...
func TestGenerateSkillRootWrite(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_skill_roots/store_write")
	payload, err := os.ReadFile(filepath.Join(directory, "skill.dql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_skill_roots/store_write",
		Name: "skill", Path: "skill.dql", Text: string(payload), Connector: "studio",
	...
	if os.Getenv("DATLY_GENERATE_SKILL_ROOT_WRITE") == "1" {
		destination = root
