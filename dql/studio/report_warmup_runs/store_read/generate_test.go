...
//go:generate env DATLY_GENERATE_WARMUP_RUN_STORE=1 go test -run TestGenerateWarmupRunStore -count=1 .
...
func TestGenerateWarmupRunStore(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_warmup_runs/store_read")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_warmup_runs/store_read",
	...
	if os.Getenv("DATLY_GENERATE_WARMUP_RUN_STORE") == "1" {
		destination = root
