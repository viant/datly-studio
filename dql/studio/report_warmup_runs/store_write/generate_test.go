...
//go:generate env DATLY_GENERATE_WARMUP_WRITE_STORE=1 go test -run TestGenerateWarmupWriteStore -count=1 .
...
func TestGenerateWarmupWriteStore(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_warmup_runs/store_write")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_warmup_runs/store_write",
	...
	if os.Getenv("DATLY_GENERATE_WARMUP_WRITE_STORE") == "1" {
		destination = root
