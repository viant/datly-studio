...
//go:generate env DATLY_GENERATE_EXPIRED_WARMUP_RUN_STORE=1 go test -run TestGenerateExpiredWarmupRunStore -count=1 .
...
func TestGenerateExpiredWarmupRunStore(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_warmup_runs/store_expired")
	payload, err := os.ReadFile(filepath.Join(directory, "warmup_run.dql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_warmup_runs/store_expired",
		Name:  "warmup_run", Path: "warmup_run.dql", Text: string(payload), Connector: "studio",
	...
	if os.Getenv("DATLY_GENERATE_EXPIRED_WARMUP_RUN_STORE") == "1" {
		destination = root
