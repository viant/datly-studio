...
//go:generate env DATLY_GENERATE_RESOURCE_FILE_WRITE=1 go test -run TestGenerateResourceFileWrite -count=1 .
...
func TestGenerateResourceFileWrite(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_resource_files/store_write")
	payload, err := os.ReadFile(filepath.Join(directory, "file.dql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_resource_files/store_write",
		Name: "file", Path: "file.dql", Text: string(payload), Connector: "studio",
	...
	if os.Getenv("DATLY_GENERATE_RESOURCE_FILE_WRITE") == "1" {
		destination = root
