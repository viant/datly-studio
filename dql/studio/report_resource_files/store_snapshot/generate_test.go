package store_snapshot_test

//go:generate env DATLY_GENERATE_RESOURCE_SNAPSHOTS=1 go test -run TestGenerateResourceSnapshots -count=1 .
...
func TestGenerateResourceSnapshots(t *testing.T) {
	...
	for _, item := range []struct{ parent, name string }{
		{"report_resource_files", "file"},
		{"report_resource_folders", "folder"},
		{"report_skill_roots", "skill"},
	} {
		directory := filepath.Join(root, "dql/studio", item.parent, "store_snapshot")
		payload, err := os.ReadFile(filepath.Join(directory, item.name+".dql"))
		if err != nil {
			t.Fatal(err)
		}
		...
		compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
			Scope: "github.com/viant/datly-studio/dql/studio/" + item.parent + "/store_snapshot",
		...
		if os.Getenv("DATLY_GENERATE_RESOURCE_SNAPSHOTS") == "1" {
			destination = root
