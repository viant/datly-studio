...
//go:generate env DATLY_GENERATE_NAMESPACE_ACCESS=1 go test -run TestGenerateNamespaceAccess -count=1 .
...
func TestGenerateNamespaceAccess(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/namespaces/store_access")
	...
	if os.Getenv("DATLY_GENERATE_NAMESPACE_ACCESS") == "1" {
		destination = root
