...
//go:generate env DATLY_GENERATE_NAMESPACE_STORES=1 go test -run TestGenerateNamespaceStores -count=1 .
...
func TestGenerateNamespaceStores(t *testing.T) {
	...
	for _, item := range []struct{ directory, name string }{{"store_read", "namespace"}, {"store_usage", "usage"}} {
		directory := filepath.Join(root, "dql/studio/namespaces", item.directory)
		...
		if os.Getenv("DATLY_GENERATE_NAMESPACE_STORES") == "1" {
			destination = root
