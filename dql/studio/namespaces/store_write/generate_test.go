...
//go:generate env DATLY_GENERATE_NAMESPACE_WRITER=1 go test -run TestGenerateNamespaceWriter -count=1 .
...
func TestGenerateNamespaceWriter(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/namespaces/store_write")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/namespaces/store_write",
	...
	if os.Getenv("DATLY_GENERATE_NAMESPACE_WRITER") == "1" {
		destination = root
