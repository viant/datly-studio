...
//go:generate env DATLY_GENERATE_NAMESPACE_INSERT=1 go test -run TestGenerateNamespaceInsert -count=1 .
...
func TestGenerateNamespaceInsert(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/namespaces/store_insert")
	payload, err := os.ReadFile(filepath.Join(directory, "namespace.dql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/namespaces/store_insert",
	...
	if os.Getenv("DATLY_GENERATE_NAMESPACE_INSERT") == "1" {
		destination = root
