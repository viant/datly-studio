...
//go:generate env DATLY_GENERATE_NAMESPACE_CLAIMS=1 go test -run TestGenerateNamespaceClaims -count=1 .
...
func TestGenerateNamespaceClaims(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/resource_namespace_claims/store_write")
	payload, err := os.ReadFile(filepath.Join(directory, "claim.dql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/resource_namespace_claims/store_write",
		Name: "claim", Path: "claim.dql", Text: string(payload), Connector: "studio",
	...
	if os.Getenv("DATLY_GENERATE_NAMESPACE_CLAIMS") == "1" {
		destination = root
