...
//go:generate env DATLY_GENERATE_CONNECTOR_CATALOG=1 go test -run TestGenerateConnectorCatalog -count=1 .
...
func TestGenerateConnectorCatalog(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/connectors/store_catalog")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/connectors/store_catalog",
	...
	if os.Getenv("DATLY_GENERATE_CONNECTOR_CATALOG") == "1" {
		destination = root
