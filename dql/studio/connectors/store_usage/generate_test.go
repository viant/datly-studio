...
//go:generate env DATLY_GENERATE_CONNECTOR_USAGE=1 go test -run TestGenerateConnectorUsage -count=1 .
...
func TestGenerateConnectorUsage(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/connectors/store_usage")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/connectors/store_usage",
	...
	if os.Getenv("DATLY_GENERATE_CONNECTOR_USAGE") == "1" {
		destination = root
