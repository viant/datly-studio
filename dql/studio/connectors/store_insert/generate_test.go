...
//go:generate env DATLY_GENERATE_CONNECTOR_INSERT=1 go test -run TestGenerateConnectorInsert -count=1 .
...
func TestGenerateConnectorInsert(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/connectors/store_insert")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/connectors/store_insert",
	...
	if os.Getenv("DATLY_GENERATE_CONNECTOR_INSERT") == "1" {
		destination = root
