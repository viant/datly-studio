...
//go:generate env DATLY_GENERATE_CONNECTOR_CONFIG=1 go test -run TestGenerateConnectorConfig -count=1 .
...
func TestGenerateConnectorConfig(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/connectors/store_config")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/connectors/store_config",
	...
	if os.Getenv("DATLY_GENERATE_CONNECTOR_CONFIG") == "1" {
		destination = root
