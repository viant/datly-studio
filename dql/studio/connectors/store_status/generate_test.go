...
//go:generate env DATLY_GENERATE_CONNECTOR_STATUS=1 go test -run TestGenerateConnectorStatus -count=1 .
...
func TestGenerateConnectorStatus(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/connectors/store_status")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/connectors/store_status",
	...
	if os.Getenv("DATLY_GENERATE_CONNECTOR_STATUS") == "1" {
		destination = root
