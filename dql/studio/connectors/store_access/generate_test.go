package store_access_test

//go:generate env DATLY_GENERATE_CONNECTOR_ACCESS=1 go test -run TestGenerateConnectorAccess -count=1 .
...
func TestGenerateConnectorAccess(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/connectors/store_access")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/connectors/store_access",
	...
	if os.Getenv("DATLY_GENERATE_CONNECTOR_ACCESS") == "1" {
		destination = root
