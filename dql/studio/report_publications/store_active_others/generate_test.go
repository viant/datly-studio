...
//go:generate env DATLY_GENERATE_ACTIVE_OTHERS=1 go test -run TestGenerateActiveOthers -count=1 .
...
func TestGenerateActiveOthers(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_publications/store_active_others")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_publications/store_active_others",
	...
	if os.Getenv("DATLY_GENERATE_ACTIVE_OTHERS") == "1" {
		destination = root
