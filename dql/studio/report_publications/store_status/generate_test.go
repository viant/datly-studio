package store_status_test

//go:generate env DATLY_GENERATE_PUBLICATION_STATUS=1 go test -run TestGeneratePublicationStatus -count=1 .
...
func TestGeneratePublicationStatus(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_publications/store_status")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_publications/store_status",
	...
	if os.Getenv("DATLY_GENERATE_PUBLICATION_STATUS") == "1" {
		destination = root
