package store_staged_test

//go:generate env DATLY_GENERATE_STAGED_PUBLICATIONS=1 go test -run TestGenerateStagedPublications -count=1 .
...
func TestGenerateStagedPublications(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_publications/store_staged")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_publications/store_staged",
	...
	if os.Getenv("DATLY_GENERATE_STAGED_PUBLICATIONS") == "1" {
		destination = root
