...
//go:generate env DATLY_GENERATE_RUNTIME_PUBLICATIONS=1 go test -run TestGenerateRuntimePublications -count=1 .
...
func TestGenerateRuntimePublications(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_publications/store_runtime_catalog")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_publications/store_runtime_catalog",
	...
	if os.Getenv("DATLY_GENERATE_RUNTIME_PUBLICATIONS") == "1" {
		destination = root
