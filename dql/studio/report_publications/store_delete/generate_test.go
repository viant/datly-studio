package store_delete_test

//go:generate env DATLY_GENERATE_PUBLICATION_DELETE=1 go test -run TestGeneratePublicationDelete -count=1 .
...
func TestGeneratePublicationDelete(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_publications/store_delete")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_publications/store_delete",
	...
	if os.Getenv("DATLY_GENERATE_PUBLICATION_DELETE") == "1" {
		destination = root
