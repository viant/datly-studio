package store_repoint_test

//go:generate env DATLY_GENERATE_PUBLICATION_REPOINT=1 go test -run TestGeneratePublicationRepoint -count=1 .
...
func TestGeneratePublicationRepoint(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_publications/store_repoint")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_publications/store_repoint",
	...
	if os.Getenv("DATLY_GENERATE_PUBLICATION_REPOINT") == "1" {
		destination = root
