...
//go:generate env DATLY_GENERATE_PUBLICATION_INSERT=1 go test -run TestGeneratePublicationInsert -count=1 .
...
func TestGeneratePublicationInsert(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_publications/store_insert")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_publications/store_insert",
	...
	if os.Getenv("DATLY_GENERATE_PUBLICATION_INSERT") == "1" {
		destination = root
