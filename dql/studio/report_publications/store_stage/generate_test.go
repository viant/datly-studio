package store_stage_test

//go:generate env DATLY_GENERATE_PUBLICATION_STAGE=1 go test -run TestGeneratePublicationStage -count=1 .
...
func TestGeneratePublicationStage(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_publications/store_stage")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_publications/store_stage",
	...
	if os.Getenv("DATLY_GENERATE_PUBLICATION_STAGE") == "1" {
		destination = root
