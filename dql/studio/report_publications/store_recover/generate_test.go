package store_recover_test

//go:generate env DATLY_GENERATE_PUBLICATION_RECOVER=1 go test -run TestGeneratePublicationRecover -count=1 .
...
func TestGeneratePublicationRecover(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_publications/store_recover")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_publications/store_recover",
	...
	if os.Getenv("DATLY_GENERATE_PUBLICATION_RECOVER") == "1" {
		destination = root
