package store_edit_test

//go:generate env DATLY_GENERATE_VERSION_EDIT=1 go test -run TestGenerateVersionEdit -count=1 .
...
func TestGenerateVersionEdit(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_versions/store_edit")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_versions/store_edit",
	...
	if os.Getenv("DATLY_GENERATE_VERSION_EDIT") == "1" {
		destination = root
