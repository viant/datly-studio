...
//go:generate env DATLY_GENERATE_VERSION_CATALOG=1 go test -run TestGenerateVersionCatalog -count=1 .
...
func TestGenerateVersionCatalog(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_versions/store_catalog")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_versions/store_catalog",
	...
	if os.Getenv("DATLY_GENERATE_VERSION_CATALOG") == "1" {
		destination = root
