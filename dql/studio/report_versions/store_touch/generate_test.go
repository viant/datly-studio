package store_touch_test

//go:generate env DATLY_GENERATE_VERSION_TOUCH=1 go test -run TestGenerateVersionTouch -count=1 .
...
func TestGenerateVersionTouch(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_versions/store_touch")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_versions/store_touch",
	...
	if os.Getenv("DATLY_GENERATE_VERSION_TOUCH") == "1" {
		destination = root
