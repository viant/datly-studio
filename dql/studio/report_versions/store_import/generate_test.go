package store_import_test

//go:generate env DATLY_GENERATE_VERSION_IMPORT=1 go test -run TestGenerateVersionImportWriter -count=1 .
...
func TestGenerateVersionImportWriter(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_versions/store_import")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_versions/store_import",
	...
	if os.Getenv("DATLY_GENERATE_VERSION_IMPORT") == "1" {
		destination = root
