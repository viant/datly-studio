...
//go:generate env DATLY_GENERATE_VERSION_HEAD=1 go test -run TestGenerateVersionHeadReader -count=1 .
...
func TestGenerateVersionHeadReader(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_versions/store_head")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_versions/store_head",
	...
	if os.Getenv("DATLY_GENERATE_VERSION_HEAD") == "1" {
		destination = root
