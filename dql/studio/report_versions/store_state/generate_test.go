...
//go:generate env DATLY_GENERATE_VERSION_STATE=1 go test -run TestGenerateVersionState -count=1 .
...
func TestGenerateVersionState(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_versions/store_state")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_versions/store_state",
	...
	if os.Getenv("DATLY_GENERATE_VERSION_STATE") == "1" {
		destination = root
