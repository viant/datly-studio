...
//go:generate env DATLY_GENERATE_VERSION_INSERT=1 go test -run TestGenerateVersionInsert -count=1 .
...
func TestGenerateVersionInsert(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_versions/store_insert")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_versions/store_insert",
	...
	if os.Getenv("DATLY_GENERATE_VERSION_INSERT") == "1" {
		destination = root
