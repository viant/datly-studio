package store_mcp_names_test

//go:generate env DATLY_GENERATE_MCP_NAMES=1 go test -run TestGenerateMCPNames -count=1 .
...
func TestGenerateMCPNames(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_versions/store_mcp_names")
	...
	if os.Getenv("DATLY_GENERATE_MCP_NAMES") == "1" {
		destination = root
