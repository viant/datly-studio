...
//go:generate env DATLY_GENERATE_RESOURCE_FOLDER_WRITE=1 go test -run TestGenerateResourceFolderWrite -count=1 .
...
func TestGenerateResourceFolderWrite(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/report_resource_folders/store_write")
	payload, err := os.ReadFile(filepath.Join(directory, "folder.dql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/report_resource_folders/store_write",
		Name: "folder", Path: "folder.dql", Text: string(payload), Connector: "studio",
	...
	if os.Getenv("DATLY_GENERATE_RESOURCE_FOLDER_WRITE") == "1" {
		destination = root
