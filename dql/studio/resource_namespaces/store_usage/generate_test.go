package store_usage_test

//go:generate env DATLY_GENERATE_RESOURCE_NAMESPACE_USAGE=1 go test -run TestGenerateResourceNamespaceUsage -count=1 .
...
func TestGenerateResourceNamespaceUsage(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/resource_namespaces/store_usage")
	payload, err := os.ReadFile(filepath.Join(directory, "usage.dql"))
	if err != nil {
		t.Fatal(err)
	}
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/resource_namespaces/store_usage",
		Name: "usage", Path: "usage.dql", Text: string(payload), Connector: "studio",
	...
	if os.Getenv("DATLY_GENERATE_RESOURCE_NAMESPACE_USAGE") == "1" {
		destination = root
