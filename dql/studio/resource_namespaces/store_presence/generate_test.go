package store_presence_test

//go:generate env DATLY_GENERATE_NAMESPACE_PRESENCE=1 go test -run TestGenerateNamespacePresence -count=1 .
...
func TestGenerateNamespacePresence(t *testing.T) {
	...
	directory := filepath.Join(root, "dql/studio/resource_namespaces/store_presence")
	...
	compiled, err := transcribe.NewCompiler().Compile(ctx, &transcribe.Source{
		Scope: "github.com/viant/datly-studio/dql/studio/resource_namespaces/store_presence",
	...
	if os.Getenv("DATLY_GENERATE_NAMESPACE_PRESENCE") == "1" {
		destination = root
