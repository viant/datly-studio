...
func FileHandlerCurrentFileIndexByContentKey(value *CurrentFileView) (string, bool) {
	var zero string
	if value == nil {
		return zero, false
	}
...
type FileHandlerCurrentFileIndexedByContent map[string]*CurrentFileView

func (rows FileHandlerCurrentFileSlice) IndexByContent() (FileHandlerCurrentFileIndexedByContent, error) {
	result := make(FileHandlerCurrentFileIndexedByContent)
	for _, row := range rows {
		key, ok := FileHandlerCurrentFileIndexByContentKey(row)
		if !ok {
			continue
		}
		if _, exists := result[key]; exists {
			return nil, fmt.Errorf("ambiguous application index FileHandlerCurrentFileSlice.IndexByContent")
		}
		result[key] = row
	}
	return result, nil
}
func (index FileHandlerCurrentFileIndexedByContent) Has(key string) bool {
	_, ok := index[key]
	return ok
}

type FileHandlerCurrentFileGroupedByContent map[string][]*CurrentFileView

func (rows FileHandlerCurrentFileSlice) GroupByContent() FileHandlerCurrentFileGroupedByContent {
	result := make(FileHandlerCurrentFileGroupedByContent)
	for _, row := range rows {
		key, ok := FileHandlerCurrentFileIndexByContentKey(row)
		if !ok {
			continue
...
func (index FileHandlerCurrentFileGroupedByContent) Has(key string) bool {
	_, ok := index[key]
	return ok
}
func FileHandlerCurrentFileIndexByContentSizeKey(value *CurrentFileView) (int, bool) {
	var zero int
	if value == nil {
		return zero, false
	}
	if value.ContentSize == nil {
		return zero, false
	}
	return *value.ContentSize, true
}

type FileHandlerCurrentFileIndexedByContentSize map[int]*CurrentFileView
...
func (index FileHandlerCurrentFileIndexedByContentSize) Has(key int) bool {
	_, ok := index[key]
	return ok
}

type FileHandlerCurrentFileGroupedByContentSize map[int][]*CurrentFileView
...
func (index FileHandlerCurrentFileGroupedByContentSize) Has(key int) bool {
	_, ok := index[key]
	return ok
}
...
func FileHandlerCurrentFileIndexByIsBinaryKey(value *CurrentFileView) (int, bool) {
	var zero int
	if value == nil {
		return zero, false
	}
	if value.IsBinary == nil {
		return zero, false
	}
	return *value.IsBinary, true
}

type FileHandlerCurrentFileIndexedByIsBinary map[int]*CurrentFileView
...
func (index FileHandlerCurrentFileIndexedByIsBinary) Has(key int) bool {
	_, ok := index[key]
	return ok
}

type FileHandlerCurrentFileGroupedByIsBinary map[int][]*CurrentFileView
...
func (index FileHandlerCurrentFileGroupedByIsBinary) Has(key int) bool {
	_, ok := index[key]
	return ok
}
