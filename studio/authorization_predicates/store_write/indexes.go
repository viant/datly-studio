...
func BuildAuthorizationPredicateHandlerReadIndexes(ctx context.Context, input *Input) (*AuthorizationPredicateHandlerReadIndexes, error) {
	if input == nil {
	...
	result := &AuthorizationPredicateHandlerReadIndexes{}
	{
		...
		inputAccess, err := xshape.Linked(reflect.TypeOf((*Input)(nil)).Elem()).Accessor("CurrentAuthorizationPredicate")
		if err != nil {
			return nil, err
		}
...
func (input *Input) ReadIndexes(ctx context.Context) (*AuthorizationPredicateHandlerReadIndexes, error) {
	if input == nil {
		return nil, fmt.Errorf("read indexes require input")
	}
