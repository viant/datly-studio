...
func PublicationHandlerCurrentPublicationIndexByActiveGenerationKey(value *CurrentPublicationView) (int64, bool) {
	var zero int64
	if value == nil {
		return zero, false
	}
...
type PublicationHandlerCurrentPublicationIndexedByActiveGeneration map[int64]*CurrentPublicationView
...
func (index PublicationHandlerCurrentPublicationIndexedByActiveGeneration) Has(key int64) bool {
	_, ok := index[key]
	return ok
}

type PublicationHandlerCurrentPublicationGroupedByActiveGeneration map[int64][]*CurrentPublicationView
...
func (index PublicationHandlerCurrentPublicationGroupedByActiveGeneration) Has(key int64) bool {
	_, ok := index[key]
	return ok
}
