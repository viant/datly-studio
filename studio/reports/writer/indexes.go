...
func ReportHandlerCurrentReportIndexByDefaultConnectorNameKey(value *CurrentReportView) (string, bool) {
	...
	if value.DefaultConnectorName == nil {
		return zero, false
	}
...
func ReportHandlerCurrentReportIndexByEtagKey(value *CurrentReportView) (int, bool) {
	var zero int
	if value == nil {
		return zero, false
	}
...
type ReportHandlerCurrentReportIndexedByEtag map[int]*CurrentReportView
...
func (index ReportHandlerCurrentReportIndexedByEtag) Has(key int) bool {
	_, ok := index[key]
	return ok
}

type ReportHandlerCurrentReportGroupedByEtag map[int][]*CurrentReportView
...
func (index ReportHandlerCurrentReportGroupedByEtag) Has(key int) bool {
	_, ok := index[key]
	return ok
}
...
func ReportHandlerCurrentReportIndexByCreatedAtKey(value *CurrentReportView) (time.Time, bool) {
	var zero time.Time
	if value == nil {
		return zero, false
	}
...
type ReportHandlerCurrentReportIndexedByCreatedAt map[time.Time]*CurrentReportView

func (rows ReportHandlerCurrentReportSlice) IndexByCreatedAt() (ReportHandlerCurrentReportIndexedByCreatedAt, error) {
	result := make(ReportHandlerCurrentReportIndexedByCreatedAt)
	for _, row := range rows {
		key, ok := ReportHandlerCurrentReportIndexByCreatedAtKey(row)
		if !ok {
			continue
		}
		if _, exists := result[key]; exists {
			return nil, fmt.Errorf("ambiguous application index ReportHandlerCurrentReportSlice.IndexByCreatedAt")
		}
		result[key] = row
	}
	return result, nil
}
func (index ReportHandlerCurrentReportIndexedByCreatedAt) Has(key time.Time) bool {
	_, ok := index[key]
	return ok
}

type ReportHandlerCurrentReportGroupedByCreatedAt map[time.Time][]*CurrentReportView

func (rows ReportHandlerCurrentReportSlice) GroupByCreatedAt() ReportHandlerCurrentReportGroupedByCreatedAt {
	result := make(ReportHandlerCurrentReportGroupedByCreatedAt)
	for _, row := range rows {
		key, ok := ReportHandlerCurrentReportIndexByCreatedAtKey(row)
		if !ok {
			continue
...
func (index ReportHandlerCurrentReportGroupedByCreatedAt) Has(key time.Time) bool {
	_, ok := index[key]
	return ok
}
	...
	result := &ReportHandlerReadIndexes{}
	{
		...
		for ordinal, row := range rows {
			...
			if (xshape.Runtime{}).IsNil(loaded) || !loaded.Has("CreatedAt") {
				return nil, fmt.Errorf("application index field was not loaded: CurrentReport.CreatedAt")
			}
