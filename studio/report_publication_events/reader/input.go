...
// reader. The event predicate restricts results to the report's current owner.
type Input struct {
	Jwt       *jwt.Claims `parameter:"Jwt,kind=header,in=Authorization,dataType=string,errorCode=401,required=true" codec:"JwtClaim"`
	ReportID  string      `parameter:"ReportID,kind=query,in=reportId,dataType=string,required=true" predicate:"equal,group=1,e,report_id" predicate:"handler,group=3,github.com/viant/datly-studio/studio/authorization.PublicationEventRead"`
	Operation string      `parameter:"Operation,kind=query,in=operation,dataType=string,required=false" predicate:"equal,group=1,e,operation"`
	Status    string      `parameter:"Status,kind=query,in=status,dataType=string,required=false" predicate:"equal,group=1,e,status"`
