...
func SessionHandlerCurrentSessionIndexByExpiresAtUnixKey(value *CurrentSessionView) (int64, bool) {
	var zero int64
	if value == nil {
		return zero, false
	}
...
type SessionHandlerCurrentSessionIndexedByExpiresAtUnix map[int64]*CurrentSessionView
...
func (index SessionHandlerCurrentSessionIndexedByExpiresAtUnix) Has(key int64) bool {
	_, ok := index[key]
	return ok
}

type SessionHandlerCurrentSessionGroupedByExpiresAtUnix map[int64][]*CurrentSessionView
...
func (index SessionHandlerCurrentSessionGroupedByExpiresAtUnix) Has(key int64) bool {
	_, ok := index[key]
	return ok
}
func SessionHandlerCurrentSessionIndexByCreatedAtKey(value *CurrentSessionView) (time.Time, bool) {
	var zero time.Time
	if value == nil {
		return zero, false
	}
...
type SessionHandlerCurrentSessionIndexedByCreatedAt map[time.Time]*CurrentSessionView

func (rows SessionHandlerCurrentSessionSlice) IndexByCreatedAt() (SessionHandlerCurrentSessionIndexedByCreatedAt, error) {
	result := make(SessionHandlerCurrentSessionIndexedByCreatedAt)
	for _, row := range rows {
		key, ok := SessionHandlerCurrentSessionIndexByCreatedAtKey(row)
		if !ok {
			continue
		}
		if _, exists := result[key]; exists {
			return nil, fmt.Errorf("ambiguous application index SessionHandlerCurrentSessionSlice.IndexByCreatedAt")
		}
		result[key] = row
	}
	return result, nil
}
func (index SessionHandlerCurrentSessionIndexedByCreatedAt) Has(key time.Time) bool {
	_, ok := index[key]
	return ok
}

type SessionHandlerCurrentSessionGroupedByCreatedAt map[time.Time][]*CurrentSessionView

func (rows SessionHandlerCurrentSessionSlice) GroupByCreatedAt() SessionHandlerCurrentSessionGroupedByCreatedAt {
	result := make(SessionHandlerCurrentSessionGroupedByCreatedAt)
	for _, row := range rows {
		key, ok := SessionHandlerCurrentSessionIndexByCreatedAtKey(row)
		if !ok {
			continue
...
func (index SessionHandlerCurrentSessionGroupedByCreatedAt) Has(key time.Time) bool {
	_, ok := index[key]
	return ok
}
...
func BuildSessionHandlerReadIndexes(ctx context.Context, input *Input) (*SessionHandlerReadIndexes, error) {
	...
	result := &SessionHandlerReadIndexes{}
	{
		...
		for ordinal, row := range rows {
			...
			if (xshape.Runtime{}).IsNil(loaded) || !loaded.Has("PayloadCiphertext") {
				return nil, fmt.Errorf("application index field was not loaded: CurrentSession.PayloadCiphertext")
			}
			...
			if (xshape.Runtime{}).IsNil(loaded) || !loaded.Has("CreatedAt") {
				return nil, fmt.Errorf("application index field was not loaded: CurrentSession.CreatedAt")
			}
		}
