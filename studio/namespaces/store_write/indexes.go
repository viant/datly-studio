...
func NamespaceHandlerCurrentNamespaceIndexByEtagKey(value *CurrentNamespaceView) (int64, bool) {
	var zero int64
	if value == nil {
		return zero, false
	}
...
type NamespaceHandlerCurrentNamespaceIndexedByEtag map[int64]*CurrentNamespaceView
...
func (index NamespaceHandlerCurrentNamespaceIndexedByEtag) Has(key int64) bool {
	_, ok := index[key]
	return ok
}

type NamespaceHandlerCurrentNamespaceGroupedByEtag map[int64][]*CurrentNamespaceView
...
func (index NamespaceHandlerCurrentNamespaceGroupedByEtag) Has(key int64) bool {
	_, ok := index[key]
	return ok
}
...
func BuildNamespaceHandlerReadIndexes(ctx context.Context, input *Input) (*NamespaceHandlerReadIndexes, error) {
	if input == nil {
	...
	result := &NamespaceHandlerReadIndexes{}
	{
		...
		inputAccess, err := xshape.Linked(reflect.TypeOf((*Input)(nil)).Elem()).Accessor("CurrentNamespace")
		if err != nil {
			return nil, err
		}
...
func (input *Input) ReadIndexes(ctx context.Context) (*NamespaceHandlerReadIndexes, error) {
	if input == nil {
		return nil, fmt.Errorf("read indexes require input")
	}
