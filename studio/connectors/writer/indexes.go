...
func ConnectorHandlerCurrentConnectorIndexByEtagKey(value *CurrentConnectorView) (int, bool) {
	var zero int
	if value == nil {
		return zero, false
	}
...
type ConnectorHandlerCurrentConnectorIndexedByEtag map[int]*CurrentConnectorView
...
func (index ConnectorHandlerCurrentConnectorIndexedByEtag) Has(key int) bool {
	_, ok := index[key]
	return ok
}

type ConnectorHandlerCurrentConnectorGroupedByEtag map[int][]*CurrentConnectorView
...
func (index ConnectorHandlerCurrentConnectorGroupedByEtag) Has(key int) bool {
	_, ok := index[key]
	return ok
}
...
func ConnectorHandlerCurrentConnectorIndexByOwnerIdKey(value *CurrentConnectorView) (string, bool) {
	var zero string
	if value == nil {
		return zero, false
	}
...
type ConnectorHandlerCurrentConnectorIndexedByOwnerId map[string]*CurrentConnectorView

func (rows ConnectorHandlerCurrentConnectorSlice) IndexByOwnerId() (ConnectorHandlerCurrentConnectorIndexedByOwnerId, error) {
	result := make(ConnectorHandlerCurrentConnectorIndexedByOwnerId)
	for _, row := range rows {
		key, ok := ConnectorHandlerCurrentConnectorIndexByOwnerIdKey(row)
		if !ok {
			continue
		}
		if _, exists := result[key]; exists {
			return nil, fmt.Errorf("ambiguous application index ConnectorHandlerCurrentConnectorSlice.IndexByOwnerId")
		}
		result[key] = row
	}
	return result, nil
}
func (index ConnectorHandlerCurrentConnectorIndexedByOwnerId) Has(key string) bool {
	_, ok := index[key]
	return ok
}

type ConnectorHandlerCurrentConnectorGroupedByOwnerId map[string][]*CurrentConnectorView

func (rows ConnectorHandlerCurrentConnectorSlice) GroupByOwnerId() ConnectorHandlerCurrentConnectorGroupedByOwnerId {
	result := make(ConnectorHandlerCurrentConnectorGroupedByOwnerId)
	for _, row := range rows {
		key, ok := ConnectorHandlerCurrentConnectorIndexByOwnerIdKey(row)
		if !ok {
			continue
...
func (index ConnectorHandlerCurrentConnectorGroupedByOwnerId) Has(key string) bool {
	_, ok := index[key]
	return ok
}
...
func ConnectorHandlerCurrentConnectorIndexByCreatedAtKey(value *CurrentConnectorView) (time.Time, bool) {
	var zero time.Time
	if value == nil {
		return zero, false
	}
...
type ConnectorHandlerCurrentConnectorIndexedByCreatedAt map[time.Time]*CurrentConnectorView

func (rows ConnectorHandlerCurrentConnectorSlice) IndexByCreatedAt() (ConnectorHandlerCurrentConnectorIndexedByCreatedAt, error) {
	result := make(ConnectorHandlerCurrentConnectorIndexedByCreatedAt)
	for _, row := range rows {
		key, ok := ConnectorHandlerCurrentConnectorIndexByCreatedAtKey(row)
		if !ok {
			continue
		}
		if _, exists := result[key]; exists {
			return nil, fmt.Errorf("ambiguous application index ConnectorHandlerCurrentConnectorSlice.IndexByCreatedAt")
		}
		result[key] = row
	}
	return result, nil
}
func (index ConnectorHandlerCurrentConnectorIndexedByCreatedAt) Has(key time.Time) bool {
	_, ok := index[key]
	return ok
}

type ConnectorHandlerCurrentConnectorGroupedByCreatedAt map[time.Time][]*CurrentConnectorView

func (rows ConnectorHandlerCurrentConnectorSlice) GroupByCreatedAt() ConnectorHandlerCurrentConnectorGroupedByCreatedAt {
	result := make(ConnectorHandlerCurrentConnectorGroupedByCreatedAt)
	for _, row := range rows {
		key, ok := ConnectorHandlerCurrentConnectorIndexByCreatedAtKey(row)
		if !ok {
			continue
...
func (index ConnectorHandlerCurrentConnectorGroupedByCreatedAt) Has(key time.Time) bool {
	_, ok := index[key]
	return ok
}
	...
	result := &ConnectorHandlerReadIndexes{}
	{
		...
		for ordinal, row := range rows {
			...
			if (xshape.Runtime{}).IsNil(loaded) || !loaded.Has("OwnerId") {
				return nil, fmt.Errorf("application index field was not loaded: CurrentConnector.OwnerId")
			}
			...
			if (xshape.Runtime{}).IsNil(loaded) || !loaded.Has("CreatedAt") {
				return nil, fmt.Errorf("application index field was not loaded: CurrentConnector.CreatedAt")
			}
