...
func (t *Transport) newResourceSnapshotReader() (*resourceSnapshotReader, error) {
...
func (t *Transport) readResourceSnapshot(ctx context.Context, reportID string, versionNo int, result *sdk.ResourceSnapshot) (err error) {
	reader, err := t.newResourceSnapshotReader()
	if err != nil {
		return internal(err)
	}
	...
	if !ok {
		return internal(fmt.Errorf("resource file reader returned %T", value))
	}
	for _, row := range fileOutput.Files {
		if row == nil || row.ReportId != reportID || row.VersionNo != versionNo {
			return internal(errors.New("resource file reader returned a mismatched row"))
		}
	...
	if !ok {
		return internal(fmt.Errorf("resource folder reader returned %T", value))
	}
	for _, row := range folderOutput.Folders {
		if row == nil || row.ReportId != reportID || row.VersionNo != versionNo {
			return internal(errors.New("resource folder reader returned a mismatched row"))
		}
	...
	if !ok {
		return internal(fmt.Errorf("skill root reader returned %T", value))
	}
	for _, row := range skillOutput.Skills {
		if row == nil || row.ReportId != reportID || row.VersionNo != versionNo {
			return internal(errors.New("skill root reader returned a mismatched row"))
		}
