...
import (
	...
	stored "github.com/viant/datly-studio/studio/report_resource_files/store_write"
...
func (t *Transport) writeResourceFile(ctx context.Context, tx *sql.Tx, row *stored.StoredFile) error {
	...
	holder := reflect.TypeOf(stored.FileComponent{})
	contract, ok := holder.FieldByName("Contract")
	if !ok {
		return fmt.Errorf("resource file writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("resource file writer has no component metadata")
	}
