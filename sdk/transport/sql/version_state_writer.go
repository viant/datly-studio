...
func (t *Transport) writeVersionState(ctx context.Context, tx *sql.Tx, operation string, targetVersionNo int, rows []*stored.StoredVersion) error {
	...
	if !ok {
		return fmt.Errorf("version state writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("version state writer has no component metadata")
	}
	...
	input.SetTargetVersionNo(targetVersionNo)
	input.SetVersions(rows)
	value, err := runtime.InvokeComponent(ctx, dexec.ComponentRequest{Target: target, Input: input})
	if err != nil {
		return err
	}
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("version state writer returned %T", value)
	}
	return nil
}
