...
func (t *Transport) writeVersionEdit(ctx context.Context, tx *sql.Tx, row *stored.StoredVersion) error {
	...
	if !ok {
		return fmt.Errorf("version edit writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("version edit writer has no component metadata")
	}
	component, err := (&bootstrap.RouteSource{HolderType: holder.Name(), FieldName: contract.Name,
		PackageName: "store_edit", PackagePath: holder.PkgPath(), Tag: metadata,
	...
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("version edit writer returned %T", value)
	}
	return nil
}
