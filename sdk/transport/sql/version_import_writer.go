...
// writeImportedVersion inserts one uploaded version and its resource files
// through the transcribed server-only Datly v1 writer. The generated lifecycle
...
func (t *Transport) writeImportedVersion(ctx context.Context, tx *sql.Tx, row *stored.ImportedVersion) error {
	...
	if !ok {
		return fmt.Errorf("version import writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("version import writer has no component metadata")
	}
	component, err := (&bootstrap.RouteSource{HolderType: holder.Name(), FieldName: contract.Name,
		PackageName: "store_import", PackagePath: holder.PkgPath(), Tag: metadata,
	...
	value, err := runtime.InvokeComponent(ctx, dexec.ComponentRequest{Target: target,
		Input: &stored.Input{Versions: []*stored.ImportedVersion{row}, Has: &stored.InputHas{Versions: true}}})
	if err != nil {
		return err
	}
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("version import writer returned %T", value)
	}
	return nil
}
