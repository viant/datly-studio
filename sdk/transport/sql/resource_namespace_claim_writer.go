...
import (
	...
	stored "github.com/viant/datly-studio/studio/resource_namespace_claims/store_write"
...
func (t *Transport) writeNamespaceClaim(ctx context.Context, tx *sql.Tx, operation string, row *stored.StoredClaim) error {
	...
	if !ok {
		return fmt.Errorf("namespace claim writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("namespace claim writer has no component metadata")
	}
	...
	input.SetClaims([]*stored.StoredClaim{row})
