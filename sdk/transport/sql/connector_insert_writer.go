...
import (
	...
	stored "github.com/viant/datly-studio/studio/connectors/store_insert"
...
func (t *Transport) writeConnectorInsertRow(ctx context.Context, row *stored.StoredConnector) error {
	...
	if !ok {
		return fmt.Errorf("connector insert writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("connector insert writer has no component metadata")
	}
	...
	value, err := runtime.InvokeComponent(ctx, dexec.ComponentRequest{Target: target,
		Input: &stored.Input{Connectors: []*stored.StoredConnector{row}, Has: &stored.InputHas{Connectors: true}}})
	if err != nil {
		return err
	}
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("connector insert writer returned %T", value)
	}
	return nil
}
