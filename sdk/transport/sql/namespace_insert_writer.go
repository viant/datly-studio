...
import (
	...
	stored "github.com/viant/datly-studio/studio/namespaces/store_insert"
...
func (t *Transport) writeNamespaceInsertRow(ctx context.Context, row *stored.StoredNamespace) error {
	...
	holder := reflect.TypeOf(stored.NamespaceComponent{})
	contract, ok := holder.FieldByName("Contract")
	if !ok {
		return fmt.Errorf("namespace insert writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("namespace insert writer has no component metadata")
	}
	...
	value, err := runtime.InvokeComponent(ctx, dexec.ComponentRequest{Target: target,
		Input: &stored.Input{Namespaces: []*stored.StoredNamespace{row}, Has: &stored.InputHas{Namespaces: true}}})
	if err != nil {
		return err
	}
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("namespace insert writer returned %T", value)
	}
	return nil
}
