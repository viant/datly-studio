...
// writeConnectorConfig invokes the server-only transcribed PATCH component.
func (t *Transport) writeConnectorConfig(ctx context.Context, row *stored.StoredConnector) error {
	...
	if !ok {
		return fmt.Errorf("connector config writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("connector config writer has no component metadata")
	}
	component, err := (&bootstrap.RouteSource{HolderType: holder.Name(), FieldName: contract.Name,
		PackageName: "store_config", PackagePath: holder.PkgPath(), Tag: metadata,
	...
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("connector config writer returned %T", value)
	}
	return nil
}
