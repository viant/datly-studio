...
func (t *Transport) writeNamespaceRow(ctx context.Context, row *stored.StoredNamespace) error {
	...
	if !ok {
		return fmt.Errorf("namespace store writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("namespace store writer has no component metadata")
	}
	...
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("namespace store writer returned %T", value)
	}
	return nil
}
