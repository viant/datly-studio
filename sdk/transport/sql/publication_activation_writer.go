...
import (
	...
	stored "github.com/viant/datly-studio/studio/report_publications/store_activate"
...
func (t *Transport) writePublicationActivation(ctx context.Context, tx *sql.Tx, versionNo int, generation int64, row *stored.StoredPublication) error {
	...
	if !ok {
		return fmt.Errorf("publication activation writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("publication activation writer has no component metadata")
	}
	component, err := (&bootstrap.RouteSource{HolderType: holder.Name(), FieldName: contract.Name,
		PackageName: "store_activate", PackagePath: holder.PkgPath(), Tag: metadata,
	...
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("publication activation writer returned %T", value)
	}
	return nil
}
