...
func (t *Transport) writeReportConfig(ctx context.Context, row *stored.StoredReport) error {
	...
	if !ok {
		return fmt.Errorf("report config writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("report config writer has no component metadata")
	}
