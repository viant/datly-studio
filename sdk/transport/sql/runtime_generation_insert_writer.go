...
func (t *Transport) writeGenerationInsert(ctx context.Context, tx *sql.Tx, row *stored.StoredGeneration) error {
	...
	if !ok {
		return fmt.Errorf("runtime generation insert writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("runtime generation insert writer has no component metadata")
	}
