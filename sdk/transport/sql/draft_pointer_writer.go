...
import (
	...
	stored "github.com/viant/datly-studio/studio/reports/store_draft_pointer"
...
// writeDraftPointer advances a report's draft pointer through the transcribed
// server-only Datly v1 writer. The row carries the report's expected etag; the
// generated lifecycle hook bumps it and the writer applies a matched update in
// the caller-owned transaction.
func (t *Transport) writeDraftPointer(ctx context.Context, tx *sql.Tx, row *stored.DraftPointer) error {
	...
	holder := reflect.TypeOf(stored.ReportComponent{})
	contract, ok := holder.FieldByName("Contract")
	if !ok {
		return fmt.Errorf("draft pointer writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("draft pointer writer has no component metadata")
	}
	component, err := (&bootstrap.RouteSource{HolderType: holder.Name(), FieldName: contract.Name,
		PackageName: "store_draft_pointer", PackagePath: holder.PkgPath(), Tag: metadata,
	...
	registration := &registry.RegisteredComponent{Component: artifact.Component, Input: artifact.Input,
		...
		Providers: []locator.Provider{views}, DataSource: dml.Source{DB: t.DB, Tx: tx}}
	...
	value, err := runtime.InvokeComponent(ctx, dexec.ComponentRequest{Target: target,
		Input: &stored.Input{Reports: []*stored.DraftPointer{row}, Has: &stored.InputHas{Reports: true}}})
	if err != nil {
		return err
	}
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("draft pointer writer returned %T", value)
	}
	return nil
}
