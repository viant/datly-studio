...
func WriteRecovery(ctx context.Context, db *sql.DB, tx *sql.Tx, operation string, row *stored.StoredPublication) error {
	input := &stored.Input{}
	input.SetOperation(operation)
	input.SetPublications([]*stored.StoredPublication{row})
	return invokeRecovery(ctx, db, tx, input)
}

func WriteCompensation(ctx context.Context, db *sql.DB, tx *sql.Tx, operation string, stagedGeneration int64, restoreStatus string, row *stored.StoredPublication) error {
	input := &stored.Input{}
	input.SetOperation(operation)
...
func invokeRecovery(ctx context.Context, db *sql.DB, tx *sql.Tx, input *stored.Input) error {
