...
// writeConnectorStatus invokes the server-only transcribed PATCH component.
// Its lifecycle hook owns the transition and Datly owns the matched write.
func (t *Transport) writeConnectorStatus(ctx context.Context, operation string, row *stored.StoredConnector) error {
	...
	if !ok {
		return fmt.Errorf("connector status writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("connector status writer has no component metadata")
	}
	component, err := (&bootstrap.RouteSource{HolderType: holder.Name(), FieldName: contract.Name,
		PackageName: "store_status", PackagePath: holder.PkgPath(), Tag: metadata,
