...
func (t *Transport) writeVersionInsert(ctx context.Context, row *stored.StoredVersion) error {
	...
	if !ok {
		return fmt.Errorf("version insert writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("version insert writer has no component metadata")
	}
	...
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("version insert writer returned %T", value)
	}
	return nil
}
