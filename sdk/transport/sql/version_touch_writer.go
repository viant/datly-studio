...
import (
	...
	stored "github.com/viant/datly-studio/studio/report_versions/store_touch"
...
func (t *Transport) writeVersionTouch(ctx context.Context, tx *sql.Tx, row *stored.StoredVersion) error {
	...
	if !ok {
		return fmt.Errorf("version touch writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("version touch writer has no component metadata")
	}
	component, err := (&bootstrap.RouteSource{HolderType: holder.Name(), FieldName: contract.Name,
		PackageName: "store_touch", PackagePath: holder.PkgPath(), Tag: metadata,
	...
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("version touch writer returned %T", value)
	}
	return nil
}
