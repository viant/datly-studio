...
func (t *Transport) writeReportInsert(ctx context.Context, row *stored.StoredReport) error {
	...
	if !ok {
		return fmt.Errorf("report insert writer has no component contract")
	}
	...
	if !present {
		return fmt.Errorf("report insert writer has no component metadata")
	}
	...
	if _, ok := value.(*stored.Output); !ok {
		return fmt.Errorf("report insert writer returned %T", value)
	}
	return nil
}
