...
export function RuntimeWorkspace({ api, mode = 'runtime', status, loading, error, onRefresh, onOpenComponent }) {
  ...
  return <main className="studio-workspace studio-runtime-workspace">
    ...
    {loading && !status ? <div className="studio-loading"><Spinner size={28}/></div> : status && <>
      {!skillsMode&&<section className="studio-generation-strip" aria-label="Recorded runtime generation"><Tag intent={status.status === 'active' ? 'success' : 'warning'}>{status.status} record</Tag><strong>{status.activeGeneration ? `Generation ${status.activeGeneration}` : 'No active generation'}</strong><span>{status.reportCount ?? 0} deployed components</span><span>{status.activatedAt ? `Activated ${formatDate(status.activatedAt)}` : 'Not activated'}</span><Tag minimal intent={hostReady ? 'success' : status.host?.status === 'unavailable' ? 'danger' : 'warning'} title={hostReady ? 'The live HTTP and MCP host responded to the runtime check.' : status.host?.status === 'unavailable' ? 'Studio could not verify that the live HTTP and MCP host is serving this generation. The publication record alone does not prove availability.' : 'No live host check is configured or its status has not been reported.'}>{hostReady ? `Live runtime ready · revision ${status.host.revision}` : status.host?.status === 'unavailable' ? 'Live runtime check failed' : 'Live runtime not checked'}</Tag></section>}
