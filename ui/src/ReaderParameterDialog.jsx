...
import { Button, ButtonGroup, Callout, Dialog, DialogBody, DialogFooter, FormGroup, HTMLSelect, InputGroup, Tag } from '@blueprintjs/core';
...
export function ReaderParameterDialog({ isOpen, structure, onClose, onApply }) {
  ...
  return <Dialog className="studio-connector-dialog studio-parameter-dialog" isOpen={isOpen} onClose={onClose} title="Inputs & constants" icon="parameter" canOutsideClickClose={!saving}>
    <form onSubmit={submit} autoComplete="off">
      <DialogBody className="studio-connector-dialog-body">
        ...
        <div className="studio-input-mode" role="tablist" aria-label="Input contract workspace"><Button type="button" small active={mode==='request'} role="tab" aria-selected={mode==='request'} onClick={()=>setMode('request')}>Request inputs <Tag minimal>{requestInputs.length}</Tag></Button><Button type="button" small active={mode==='constant'} role="tab" aria-selected={mode==='constant'} onClick={()=>setMode('constant')}>Trusted constants <Tag minimal>{constants.length}</Tag></Button></div>
      ...
      <DialogFooter actions={<><Button onClick={onClose} disabled={saving}>Close</Button><Button type="submit" intent="primary" loading={saving}>{existingName?`Save ${mode==='constant'?'constant':'input'}`:mode==='constant'?'Add constant':'Add input'}</Button></>}/>
    </form>
  </Dialog>;
}
