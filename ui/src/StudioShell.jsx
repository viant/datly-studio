...
export function StudioShell({ config, extensions = noExtensions }) {
  ...
  if (session.state === 'ready') return <StudioApp api={api} mode={config.mode} subject={session.subject} extensions={extensions}/>;
  if (session.state === 'loading') return <AuthWindow><Spinner size={30}/><h1>Checking Studio session</h1><p>Restoring your secure server-side session.</p></AuthWindow>;
  ...
  return <AuthWindow><div className="studio-auth-mark">D</div><h1>Sign in to Datly Studio</h1><p>Your session is missing or expired. Authentication continues through the deployment identity provider; tokens are never stored in this browser.</p><a className="bp6-button bp6-intent-primary bp6-large" href={sessionURL(config, config.authentication.loginPath)}><span className="bp6-button-text">Continue to sign in</span></a><Button minimal icon="refresh" onClick={inspectSession}>Check session again</Button></AuthWindow>;
}
